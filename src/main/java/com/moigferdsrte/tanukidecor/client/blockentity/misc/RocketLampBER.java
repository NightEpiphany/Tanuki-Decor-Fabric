...
import com.moigferdsrte.tanukidecor.block.entity.RocketLampBlockEntity;
...
public class RocketLampBER implements BlockEntityRenderer<RocketLampBlockEntity> {
    ...
    protected static ResourceLocation getWaxModel(final WaxSize waxSize, final String color) {
        return WAX_MODELS
                ...
                .computeIfAbsent(waxSize, size -> ResourceLocation.fromNamespaceAndPath(TanukiDecorFabric.MOD_ID, "block/rocket_lamp/" + color + "/" + size.getSerializedName() + "_wax"));
    }
    ...
    public void render(RocketLampBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        BakedModel largeModel = mc.getModelManager().getModel(TanukiDecorFabricClient.standalone(getWaxModel(WaxSize.LARGE, color)));
        BakedModel mediumModel = mc.getModelManager().getModel(TanukiDecorFabricClient.standalone(getWaxModel(WaxSize.MEDIUM, color)));
        BakedModel smallModel = mc.getModelManager().getModel(TanukiDecorFabricClient.standalone(getWaxModel(WaxSize.SMALL, color)));
