...
public final class ClientRecipeCollections {
    ...
    public static void register() {
        IEvent.CALLBACK.register(e -> {
            if (e instanceof RecipesUpdatedEvent event) {
                onUpdateRecipes(event);
            }
            if (e instanceof TagsUpdatedEvent event) {
                onUpdateTags(event);
            }
        });
    }
    ...
    private static void onUpdateRecipes(final RecipesUpdatedEvent event) {
        ...
        var diyRecipes = event.getRecipeManager().getAllRecipesFor(TDRegistry.RecipeReg.DIY);
